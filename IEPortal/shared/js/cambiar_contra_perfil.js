(function(){
	...
	var formulario = document.getElementById('cambiar_cp'),
        contrasena = formulario.password1,
        contrasena2 = formulario.password2,
        nuevac = formulario.password_anterior;   
	
    function validarContrasena(e){
        if(nuevac.value == '' || nuevac.value == null){
            //console.log('Por favor completa el campo de contraseña anterior');
            error.style.display = 'block';
            error.innerHTML += '<li>Por favor completa el campo de contraseña anterior</li>'
            e.preventDefault();
        }
    ...
    function validarFormulario(e){
		...
		if (contenido_1== ''){
		 ...
		 //console.log(parametros);
			 $.ajax({
					type: "POST",
					url: "ajax/cambiar_contrasenia_perfil.php",
					data: parametros,
