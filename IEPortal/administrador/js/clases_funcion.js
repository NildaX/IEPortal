...
function MostrarAlumnos(id_c1) {
	...
	$.ajax({
		...
		url: "ajax/pedir_alumnos_copy.php", //url de donde obtener los datos
...
function cambioTipo(e){
	...
	case 'preguntas':
		...
		$.ajax({
			data: parametros, //ajuntamos los parametros con los datos
			url: "ajax/pedir_temas_copy.php", //url de donde obtener los datos
			dataType: 'json', //tipo de datos retornados
			type: 'post' //enviar variables como post
		}).done(function (data) {
			let json_string = JSON.stringify(data);
...
document.addEventListener("DOMContentLoaded", function (event) {
	...
	$.ajax({
		...
		for (let i in obj) {
			...
			let celda2 = fila.insertCell();
			celda2.innerHTML = `${obj[i].nombre} ${obj[i].apellido_p} ${obj[i].apellido_m}`;
			let celda3 = fila.insertCell();
			celda3.innerHTML = `${obj[i].dias}`;
			let celda4 = fila.insertCell();
			celda4.innerHTML = `${obj[i].horas}`;
			let celda5 = fila.insertCell();
			celda5.innerHTML = `<button class="boton boton--pequeno" type="button" id=${obj[i].ids} onclick=MostrarAlumnos(this.id) >Mostrar Alumnos`;
		}
